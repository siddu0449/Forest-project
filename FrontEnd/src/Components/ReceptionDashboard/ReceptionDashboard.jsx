...
export default function ReceptionDashboard() {
  ...
  useEffect(() => {
    ...
    // ✅ FIX OLD / BROKEN VEHICLE DATA
    const fixedVehicles = storedVehicles.map(v => ({
      ...v,
      passengers: Array.isArray(v.passengers) ? v.passengers : [],
      seatsFilled: v.seatsFilled || 0,
    ...
    localStorage.setItem("vehicles", JSON.stringify(fixedVehicles));
  }, []);
  ...
  const autoAssignVehicle = visitor => {
    setVehicles(prev => {
      ...
      while (remainingSeats > 0) {
        let vehicle = updated.find(
          v => v.seatsFilled < 6 && v.status !== "moved"
        );
